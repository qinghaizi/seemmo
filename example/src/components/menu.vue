<template>
  ...
  >
    ...
    <el-submenu
      ...
      :data-router="
        '/main/' + item.children[0].attributes.menuPath.split('.')[0]
      "
    >
      <template slot="title"
        >{{ item.text }}
      ...
      <el-menu-item
        :index="'/main/' + i.attributes.menuPath.split('.')[0]"
        v-for="i in item.children"
        :key="i.id"
        :disabled="
          i.attributes.menuPath !== ('faceLibQuery.html' || 'channel.html')
        "
        :data-router="'/main/' + i.attributes.menuPath.split('.')[0]"
        >{{ i.text }}
      </el-menu-item>
    </el-submenu>
