...
export default class SeeMap extends mapboxgl.Map {
  ...
  public constructor(mapdivOrId: string | Element, options: SeemapOptions) {
    ...
    this._businessLayerGroup = new LayerGroup({
      title: "业务图层",
      groupIndex: 1
    }).addTo(this);
    this._topLayerGroup = new LayerGroup({
      title: "置顶图层",
      groupIndex: 2
    }).addTo(this);
  ...
  /** 切换底图样式 */
  public changeMapStyle(layerTypeOrUrl: string): void {
    // 其实都是remove后addlayer
  ...
  public moveToExtent(willMoveExtent: number[]): this {
    ...
    this.fitBounds(
      [
        [willMoveExtent[0], willMoveExtent[1]],
        [willMoveExtent[2], willMoveExtent[3]]
      ],
      {}
    );
    return this;
   ...
   */
  public setPointTips(
    coordinate?: mapboxgl.LngLatLike | undefined | null,
    innerHtml?: string
  ): void {
    if (coordinate && innerHtml) {
      this.pointTips.setLngLat(coordinate).setHTML(innerHtml);
