...
export default class SeeGate extends SeeLayer {
    ...
    constructor(setOptions: SeeGateParams = {}, styleJson?: GateStyle | GateStyle[]) {
        super(assign(GATEBASEOPTION, setOptions), styleJson || GATEBASESTYLE)

        this._enableShowLabel = setOptions.enableShowLabel ? setOptions.enableShowLabel : this._enableShowLabel
    ...
    private _featureClickFunc(evt_obj: LayerEvent): void {
        if (this.seeMap.getZoom() < this._clickMinZoom || !evt_obj.features) { return }
        // 选中和未选中
        ...
        evt_obj.features.forEach(eachFeature => {
            ...
            if (isSelected) {
                ...
                let oldIndex = this._selectedGateIds.indexOf(eachFeature.properties[this._constKey.id])
                if (oldIndex > -1) {
        ...
        this.setFeaturesProperties(selectedGates.map(each => {return each.id}) as (string | number)[], 'selected', true)
        this.setFeaturesProperties(selectedGates.map(each => {return each.id}) as (string | number)[], '_icon', 'click')
        this.setFeaturesProperties(unSelectedGates.map(each => {return each.id}) as (string | number)[], 'selected', false)
        this.setFeaturesProperties(unSelectedGates.map(each => {return each.id}) as (string | number)[], '_icon', 'normal')

        this.dispatchEvent(
            new GateEvent('gateSelect', {features: selectedGates, coordinate: evt_obj.coordinate, message: '选中', source: evt_obj.source})
        )
        this.dispatchEvent(
            new GateEvent('gateUnSelect', {features: unSelectedGates, coordinate: evt_obj.coordinate, message: '取消选中', source: evt_obj.source})
        )
    }
    ...
    private _featureMoveOnFunc(evt_obj: LayerEvent): void {
        if (evt_obj.features) {
            ...
            if (!this._enableShowLabel) { return }
            ...
            if (showLabelText) {
                ...
                this.seeMap.setPointTips(geometry.coordinates as ([number, number]), showLabelText)
            }
            this.dispatchEvent(
                new GateEvent('gateMoveOn', {features: evt_obj.features, coordinate: evt_obj.coordinate, message: '鼠标移入', source: evt_obj.source})
            )
        }
    }
    ...
    public addGateFeatures(gateList: { [param: string]: any }[] = []): this {
        ...
        let featurePoins: { id: string | number, geometry: GeoJSON.Geometry, properties?: GeoJSON.GeoJsonProperties }[] = []
        gateList.forEach((eachGate) => {
            if (eachGate.hasOwnProperty(LATKEY) && eachGate.hasOwnProperty(LONKEY)) {
                // 判断是否添加过，如果添加过不重复添加
                if (eachGate.hasOwnProperty(GATEID) && this._allGateIds.indexOf(eachGate[GATEID].toString()) === -1) {
    ...
    public setSelectedGates(selecteds: (string | number)[] = []): this {
        ...
        selecteds.forEach((eachId) => { stringIds.push(eachId.toString()) })
        ...
        allGates.forEach(eachGeojsonGate => {
            ...
            if (eachGeojsonGate.id && stringIds.indexOf(eachGeojsonGate.id.toString()) > -1) {
     ...
     */
    public calcGateInPolygon(poplygon: GeoJSON.Polygon | GeoJSON.MultiPolygon): (string | number)[] {
