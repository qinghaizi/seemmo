 ...
 * @LastEditTime: 2019-05-10 11:18:32
 */
...
export type LAYERTYPE = 'background' | 'fill' | 'line' | 'symbol' | 'raster' | 'circle' | 'fill-extrusion' | 'heatmap' | 'hillshade'
...
export default abstract class BaseLayer extends Observable {
  ...
  // 封装内部事件监听
  private _mapboxListeners: {[propName in MAPBOXLISTENERTYPE]?: Array<(ev: MapLayerEventType[MAPBOXLISTENERTYPE] & EventData) => void>} = {}

  constructor (setOptions: BaseLayerParams) {
    super()
    this.id = this.uuid + '-' + (setOptions.key ? setOptions.key : '') + '-' + new Date().getTime().toString()
   ...
   */
  public onMapboxListeners({ type, listener }: { type: MAPBOXLISTENERTYPE; listener: (ev: MapLayerEventType[MAPBOXLISTENERTYPE] & EventData) => void; }): void {
   ...
   * @param listener 监听回调方法
   */
  public offMapboxListeners({ type, listener }: { type: MAPBOXLISTENERTYPE; listener: (ev: MapLayerEventType[MAPBOXLISTENERTYPE] & EventData) => void; }): void {
  ...
  public cleanMapboxListeners(): void {
    for (const type in this._mapboxListeners) {
      const eachTypeListeners = this._mapboxListeners[(type as MAPBOXLISTENERTYPE)]
      if (!eachTypeListeners) {
        continue
      }
      eachTypeListeners.forEach(eachListener => {
        this.offMapboxListeners({ type: (type as MAPBOXLISTENERTYPE), listener: eachListener })
      })
    }
  }
  ...
  public _setSeeLayerGroup(layerGroup: LayerGroup | null): this {
    ...
    this._layers.forEach(eachLayer => {
      eachLayer.metadata.groupIndex = this._seeLayerGroup ? this._seeLayerGroup.groupIndex : undefined
    })
    return this
  }
  ...
  static _findBeforLayerId(layer: BaseLayer): string | undefined {
    ...
    for ( findIndex = 0; findIndex < lastSeemapLayer.length; findIndex++) {
      ...
      if (element.metadata.typeIndex > layer.typeIndex) {
        ...
        if (element.metadata.typeIndex === layer.typeIndex && element.metadata.index > layer.index) {
          break
        }
      }
    }
